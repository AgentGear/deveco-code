...
function setFdNonblock(fd: number): (() => void) | undefined {
  try {
    // Bun exposes fcntlSync + F_GETFL/F_SETFL on Linux only; type defs don't declare them.
    const fsAny = fs as unknown as {
      fcntlSync?: (fd: number, cmd: number, arg?: number) => number
      constants: Record<string, number | undefined>
    }
    const { fcntlSync, constants } = fsAny
    if (!fcntlSync || !constants?.O_NONBLOCK) return undefined
    const F_GETFL = constants.F_GETFL
    const F_SETFL = constants.F_SETFL
    if (F_GETFL === undefined || F_SETFL === undefined) return undefined
    const flags = fcntlSync(fd, F_GETFL)
    fcntlSync(fd, F_SETFL, flags | (constants.O_NONBLOCK ?? 0))
    return () => {
      fcntlSync(fd, F_SETFL, flags)
    }
  } catch {
    return undefined
  }
