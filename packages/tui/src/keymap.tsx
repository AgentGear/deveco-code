...
export const DEVECO_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const DEVECO_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(DEVECO_MODE_KEY, DEVECO_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(DEVECO_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(DEVECO_MODE_KEY, stack.at(-1)?.mode ?? DEVECO_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? DEVECO_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(DEVECO_MODE_KEY, undefined)
