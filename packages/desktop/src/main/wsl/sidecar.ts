...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export DEVECO_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export DEVECO_CLIENT=desktop",
    `export DEVECO_SERVER_USERNAME=${shellEscape(username)}`,
    `export DEVECO_SERVER_PASSWORD=${shellEscape(password)}`,
