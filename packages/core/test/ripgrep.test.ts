...
describe("Ripgrep", () => {
  ...
  it.live("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".deveco")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".deveco", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".deveco/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".deveco/config"))
