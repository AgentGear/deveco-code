...
describe("OpencodePlugin", () => {
  it.effect("uses a public key and cancels paid models without credentials", () =>
    withEnv({ DEVECO_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        const updated = yield* plugin.trigger("provider.update", {}, { provider: provider("deveco"), cancel: false })
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ DEVECO_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        yield* plugin.trigger("provider.update", {}, { provider: provider("deveco"), cancel: false })
  ...
  it.effect("uses DEVECO_API_KEY as credentials", () =>
    withEnv({ DEVECO_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        const updated = yield* plugin.trigger("provider.update", {}, { provider: provider("deveco"), cancel: false })
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ DEVECO_API_KEY: undefined, CUSTOM_DEVECO_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(OpencodePlugin)
        const updated = yield* plugin.trigger(
          "provider.update",
          {},
          { provider: provider("deveco", { env: ["CUSTOM_DEVECO_API_KEY"] }), cancel: false },
        )
        const paid = yield* plugin.trigger(
          "model.update",
          {},
    ...
    withEnv({ DEVECO_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        const updated = yield* plugin.trigger(
          "provider.update",
          {},
          { provider: provider("deveco", { enabled: { via: "auth", service: "opencode" } }), cancel: false },
        )
        const paid = yield* plugin.trigger(
          "model.update",
          {},
