...
describe("ModelsDevPlugin", () => {
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.DEVECO_MODELS_PATH,
          disabled: Flag.DEVECO_DISABLE_MODELS_FETCH,
        }
        Flag.DEVECO_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.DEVECO_MODELS_PATH = previous.path
          Flag.DEVECO_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
