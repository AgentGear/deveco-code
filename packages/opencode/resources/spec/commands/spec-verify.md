...
1. **Evaluate Initial Results** (from Phase 1):
    ...
    - **Build failures remain** → Continue build fix loop from Phase 1 step 1, respecting the 10-attempt cap (`build_fix_attempts`). If `build_fix_attempts ≥ 10`, output `[BUILD_LIMIT_EXCEEDED]` and proceed directly to step 5 (Report) with failure status. If build succeeds after a fix, run `start_app`, then evaluate UI results (if `build+ui`) or proceed to step 5 (Report).
...
3. **Build-Only Fix Cycle** (when `Verification_Scope == build-only` and build is still failing):
    Apply the same fix → rebuild → redeploy loop from Phase 1 step 1, respecting the 10-attempt cap (`build_fix_attempts`). Each re-build must increment `build_fix_attempts` and output `[BUILD_ATTEMPT] attempt=<build_fix_attempts>/10`. If build succeeds, run `start_app` and proceed to step 5 (Report). If `build_fix_attempts ≥ 10` and the build still fails, output `[BUILD_LIMIT_EXCEEDED]` and proceed to step 5 (Report) with failure status.
...
7. **Self-Audit Rule**: Before writing the Report (step 5), review your own output and verify that: (a) every `build_project` invocation was preceded by a `[BUILD_ATTEMPT]` assertion and `build_fix_attempts` never exceeded 10, (b) every `verify_ui` invocation in Phases 1–2 was preceded by a `[VERIFY_UI_CALL]` assertion and no `verification_attempts[story]` exceeded 3, (c) Phase 3 invoked `verify_ui` exactly once per story with `freshStart=true` and no code fixes were applied during Phase 3, (d) the Report reflects Phase 3 results as the authoritative final status. If you discover a violation, correct the report to reflect the actual counts and flag the violation explicitly: `[AUDIT_VIOLATION] <subject> exceeded limit (actual: <count>, limit: <max>)`.
