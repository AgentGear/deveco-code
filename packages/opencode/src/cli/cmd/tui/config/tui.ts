...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old deveco.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.deveco`
  // folders between cwd and home, and DEVECO_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.DEVECO_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit DEVECO_TUI_CONFIG override, if set.
  if (Flag.DEVECO_TUI_CONFIG) {
    const configFile = Flag.DEVECO_TUI_CONFIG
  ...
  // 4. `.deveco` directories (and DEVECO_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".deveco") || dir === Flag.DEVECO_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".deveco") && dir !== Flag.DEVECO_CONFIG_DIR) continue
