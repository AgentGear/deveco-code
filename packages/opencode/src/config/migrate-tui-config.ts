...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.CODEGENIE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("codegenie", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "codegenie")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "codegenie"))
  }
  if (Flag.CODEGENIE_CONFIG) files.push(Flag.CODEGENIE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "codegenie"))
