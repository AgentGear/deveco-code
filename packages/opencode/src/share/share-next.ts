...
const disabled = process.env["CODEGENIE_DISABLE_SHARE"] === "true" || process.env["CODEGENIE_DISABLE_SHARE"] === "1"
    ...
    const full = Effect.fn("ShareNext.full")(function* (sessionID: SessionID) {
      ...
      yield* sync(sessionID, [
        ...
        ...messages.flatMap((item) =>
          item.parts
            .filter((part) => part.type !== "queue")
            .map((part) => ({ type: "part" as const, data: part as unknown as SDK.Part })),
        ),
