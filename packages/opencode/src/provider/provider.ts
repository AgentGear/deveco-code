...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://opencode.ai/",
            "X-Title": "opencode",
            "X-BILLING-INVOKE-ORIGIN": "DevEco Code",
          },
        },
      }),
    vercel: () =>
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `deveco auth cloudflare-ai-gateway`.",
        )
      }
    ...
    deveco: Effect.fnUntraced(function* () {
      ...
      if (!accessToken) {
        yield* Effect.logWarning("deveco token unavailable, user needs to re-login")
        return { autoload: false }
      }
      ...
      Effect.gen(function* () {
        ...
        if (devecoAuth && devecoAuth.type === "oauth") {
          ...
          if (accessToken) {
            ...
            configProviders.push(["deveco", config as unknown as Partial<Info>])
          }
        }
