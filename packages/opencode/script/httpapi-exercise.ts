 ...
 *
 * The script intentionally isolates `CODEGENIE_DB` before importing modules that touch
...
function app(modules: Runtime, backend: Backend) {
  Flag.CODEGENIE_EXPERIMENTAL_HTTPAPI = backend === "effect"
    ...
    modules.ExperimentalHttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ CODEGENIE_SERVER_PASSWORD: undefined, CODEGENIE_SERVER_USERNAME: undefined }),
        ),
      ),
    ),
    { disableLogger: true },
