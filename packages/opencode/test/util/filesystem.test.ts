...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "deveco.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "deveco.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "deveco.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "deveco.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["deveco.json", "deveco.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "deveco.json"),
        path.join(project, "deveco.jsonc"),
      ])
    })
  })
