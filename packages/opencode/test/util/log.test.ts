...
it.live("local dev log is not truncated twice for the same run", () =>
  Effect.gen(function* () {
    ...
    const initialized = process.env.DEVECO_LOG_INITIALIZED_RUN_ID
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Global.Path.log = log
        if (runID === undefined) delete process.env.DEVECO_RUN_ID
        else process.env.DEVECO_RUN_ID = runID
        if (initialized === undefined) delete process.env.DEVECO_LOG_INITIALIZED_RUN_ID
        else process.env.DEVECO_LOG_INITIALIZED_RUN_ID = initialized
      }),
    )

    const dir = yield* tmpdirScoped()
    Global.Path.log = dir
    process.env.DEVECO_RUN_ID = "run-1"
    delete process.env.DEVECO_LOG_INITIALIZED_RUN_ID
