...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.DEVECO_EXPERIMENTAL_DISABLE_FILEWATCHER
    ...
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.DEVECO_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.DEVECO_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
