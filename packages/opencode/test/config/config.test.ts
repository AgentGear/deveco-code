...
async function writeManagedSettings(settings: object, filename = "deveco.json") {
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "deveco.jsonc"))
...
test("does not create global config when DEVECO_CONFIG_DIR is set", async () => {
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "deveco.jsonc"))).toBe(false)
  } finally {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "deveco.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "deveco.jsonc")
  ...
  try {
    ...
    await withTestInstance({
      ...
      fn: async (ctx) => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "deveco.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "deveco.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(opencodeDir, "deveco.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(remoteFetchedUrl).toBe("https://config.example.com/deveco.json")
