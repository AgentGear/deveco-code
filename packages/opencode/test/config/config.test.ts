...
test("does not create global config when DEVECO_CONFIG_DIR is set", async () => {
  await using tmp = await tmpdir()
  await using custom = await tmpdir()
  const prevConfig = Global.Path.config
  const prevEnv = process.env.DEVECO_CONFIG_DIR
  ;(Global.Path as { config: string }).config = tmp.path
  process.env.DEVECO_CONFIG_DIR = custom.path
  await clear(true)
  ...
  try {
    ...
    if (prevEnv === undefined) delete process.env.DEVECO_CONFIG_DIR
    else process.env.DEVECO_CONFIG_DIR = prevEnv
    await clear(true)
  }
})
...
it.instance("loads config from .opencode directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".deveco", "agent"))
...
it.instance("loads agents from .opencode/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".deveco", "agents", "nested"))
    yield* writeTextEffect(
...
it.instance("loads commands from .opencode/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".deveco", "command", "nested"))
    yield* writeTextEffect(
...
it.instance("loads commands from .opencode/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".deveco", "commands", "nested"))
    yield* writeTextEffect(
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const opencodeDir = path.join(projectDir, ".deveco")
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".deveco")
...
it.instance("local .opencode config can override MCP from project config", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".deveco"))
    yield* writeConfigEffect(
...
describe("DEVECO_DISABLE_PROJECT_CONFIG", () => {
  ...
  it.instance("skips project .deveco/ directories when flag is set", () =>
    withProcessEnv(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* mkdirEffect(path.join(test.directory, ".deveco", "command"))
        yield* writeTextEffect(
