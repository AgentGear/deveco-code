...
const PROVIDERS = {
  openai: {
    ...
    canRecord: Boolean(process.env.OPENAI_API_KEY ?? process.env.OPENAI_API_KEY),
    config: (model) => ({
      ...
      provider: {
        openai: {
          ...
          options: {
            apiKey: process.env.OPENAI_API_KEY ?? process.env.OPENAI_API_KEY ?? "fixture-openai-key",
  ...
  opencode: {
    ...
    canRecord: Boolean(process.env.CONSOLE_TOKEN && process.env.ZEN_ORG_ID),
    config: (model) => ({
      ...
      provider: {
        opencode: {
          ...
          api: zenURL(process.env.ZEN_CONNECTION ?? "fixture"),
          models: { [model.id]: cloneModel(model) },
          options: {
            apiKey: process.env.CONSOLE_TOKEN ?? "fixture-console-token",
            headers: { "x-org-id": process.env.ZEN_ORG_ID ?? "fixture-org" },
          },
        },
      },
    }),
  ...
  anthropic: {
    ...
    tags: ["opencode", "native", "tool-loop"],
    canRecord: Boolean(process.env.ANTHROPIC_API_KEY ?? process.env.ANTHROPIC_API_KEY),
    config: (model) => ({
      ...
      provider: {
        anthropic: {
          ...
          options: {
            apiKey:
              process.env.ANTHROPIC_API_KEY ?? process.env.ANTHROPIC_API_KEY ?? "fixture-anthropic-key",
