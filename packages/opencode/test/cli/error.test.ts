...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/deveco.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/deveco.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/deveco.jsonc", dir: ".opencode", suggestion: "opencode" },
        expected:
          'Directory ".opencode" in /tmp/deveco.jsonc is not valid. Rename the directory to "opencode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/deveco.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/deveco.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
