...
const originalOpencodePort = process.env.DEVECO_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.DEVECO_EDITOR_SSE_PORT = originalOpencodePort
})
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.DEVECO_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.DEVECO_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with DEVECO_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.DEVECO_EDITOR_SSE_PORT = "4020"
