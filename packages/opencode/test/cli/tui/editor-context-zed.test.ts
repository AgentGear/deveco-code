...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.CODEGENIE_ZED_DB
  process.env.CODEGENIE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.CODEGENIE_ZED_DB
    else process.env.CODEGENIE_ZED_DB = previous
    home.mockRestore()
  }
})
